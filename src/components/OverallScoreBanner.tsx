  ...
  return (
    <div className={`rounded-2xl border p-5 sm:p-6 bg-gradient-to-br ${details.bgGradient} shadow-xl relative overflow-hidden transition-all duration-300`}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row lg:flex-col items-start lg:items-end justify-between gap-3 shrink-0 border-t lg:border-t-0 border-slate-800/80 pt-4 lg:pt-0">
          ...
          <div className="flex items-center gap-2">
            {onOpenShareModal && (
              <button
                id="btn-share-scorecard-banner"
                onClick={onOpenShareModal}
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-700 hover:border-slate-600 transition shadow-sm cursor-pointer"
      ...
      <div className="mt-5 pt-4 border-t border-slate-800/80 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5 sm:gap-3">
        ...
        {/* Metric 4: KENP Reads or Print Cost */}
        <div className="bg-slate-950/60 rounded-xl p-2.5 border border-slate-800/80">
          <div className="flex items-center gap-1.5 text-slate-400 text-[11px]">
            <BookOpen className="w-3 h-3 text-purple-400" />
            <span>{metrics.isLowContent ? 'Print Cost & Margin' : 'KENP Reads'}</span>
          </div>
          <div className="text-sm font-bold text-white mt-1">
            {metrics.isLowContent ? `$${computed.printCost || 0} Print` : metrics.kenpReads.toLocaleString()}
          </div>
          <div className="text-[10px] text-slate-400">
            {metrics.isLowContent ? `~$${computed.royaltyPerUnit || 0} royalty/unit` : `~$${computed.kenpRoyalties} KU Pay`}
          </div>
        </div>
        ...
        <div className="bg-slate-950/60 rounded-xl p-2.5 border border-slate-800/80">
          ...
          <div className="text-[10px] text-slate-400">
            {metrics.isLowContent ? 'Paperback Net' : 'Direct + KU'}
          </div>
        </div>
