...
export function calculateKdpPrintCost(
  pageCount: number, 
  interiorColor: 'black_white' | 'standard_color' | 'premium_color' = 'black_white'
): number {
  const pages = Math.max(24, pageCount || 120);
  if (interiorColor === 'standard_color') {
    return Math.max(2.50, Number((1.00 + pages * 0.038).toFixed(2)));
  }
  if (interiorColor === 'premium_color') {
    return Math.max(3.00, Number((1.00 + pages * 0.075).toFixed(2)));
  }
  // Black and white interior
  if (pages < 110) {
    return 2.30;
  }
  return Number((1.00 + pages * 0.015).toFixed(2));
}
...
export function calculateComputedMetrics(metrics: BookMetrics): ComputedMetrics {
  ...
  const pageCount = Math.max(24, metrics.pageCount || (metrics.isLowContent ? 120 : 300));
  ...
  let directSalesRoyalties = 0;
  let kenpRoyalties = 0;
  let printCost = 0;
  let royaltyPerUnit = 0;
  let breakevenAcos = (metrics.royaltyRate || 0.7) * 100;
  let maxBreakevenCpc = 0;

  if (metrics.isLowContent) {
    printCost = metrics.printCost ?? calculateKdpPrintCost(pageCount, metrics.interiorColor || 'black_white');
    // KDP paperback standard formula: 60% of list price minus print cost
    const retailPrice = price || metrics.paperbackPrice || 7.99;
    royaltyPerUnit = Math.max(0, Number(((retailPrice * 0.60) - printCost).toFixed(2)));
    directSalesRoyalties = Number((orders * royaltyPerUnit).toFixed(2));
    kenpRoyalties = 0;
    breakevenAcos = retailPrice > 0 ? Number(((royaltyPerUnit / retailPrice) * 100).toFixed(1)) : 0;
    maxBreakevenCpc = clicks > 0 && orders > 0 ? Number(((orders / clicks) * royaltyPerUnit).toFixed(2)) : 0;
  } else {
    // Average Amazon KDP Global Fund KENP rate ~$0.0044 per page read
    const kenpRate = 0.0044;
    kenpRoyalties = kenp * kenpRate;
    directSalesRoyalties = orders * price * (metrics.royaltyRate || 0.7);
  }
  ...
  const estimatedKenpBorrows = metrics.isLowContent ? 0 : Math.round(kenp / pageCount);
...
export function evaluateScorecard(metrics: BookMetrics, computed: ComputedMetrics): {
  ...
  // 4. Reader Engagement & KENP Reads (or Interior Layout & A+ Content for Low-Content)
  let kenpScore = 0;
  if (metrics.isLowContent) {
    if (metrics.hasAplusContent) {
      kenpScore = 88;
    } else {
      kenpScore = 28; // Without A+ content, low content books bleed because buyers can't see the interior
    }
  } else {
  ...
  const sections: ScoreItem[] = [
    ...
    },
    {
      ...
      headline: computed.acos > 65
        ...
        : computed.acos > (metrics.isLowContent ? 35 : 45)
        ? `Marginal Ad Efficiency (ACoS ${metrics.isLowContent ? '35-65%' : '45-65%'})`
        : `Profitable / Efficient Advertising (ACoS ≤ ${metrics.isLowContent ? '35%' : '45%'})`,
      benchmark: metrics.isLowContent 
        ? `Breakeven ACoS: ~${computed.breakevenAcos}% (Paperback 60% minus Print Cost)`
        : `Breakeven ACoS: ~${computed.breakevenAcos}% (eBook 70% tier)`,
      valueDisplay: `${computed.acos}% ACoS ($${metrics.adSpend} Spend)`,
      whatYouAreMissing: computed.acos > (metrics.isLowContent ? 35 : 65)
        ? metrics.isLowContent
          ? `On a low-content paperback with $${computed.printCost || 2.80} print cost, your net royalty is only $${computed.royaltyPerUnit || 2.00}/unit. With ACoS at ${computed.acos}%, ad costs wipe out 100% of profit!`
        ...
        : computed.acos > (metrics.isLowContent ? 25 : 45)
        ? 'Ads are barely breaking even. You need to trim non-converting search terms and negotiate lower bids on competitive comp targets.'
        : 'Healthy ad margins! You can confidently increase daily ad budget by 15-20% to gain higher Amazon BSR organic ranking.',
      prescription: computed.acos > (metrics.isLowContent ? 35 : 65)
        ? metrics.isLowContent
          ? `1. Lower your maximum bid to ≤ $${computed.maxBreakevenCpc || 0.25} (your breakeven CPC). 2. Add negative keywords for low-intent search terms.`
      ...
      subMetrics: [
        {
          ...
          value: `${computed.acos}%`,
          status: computed.acos <= (metrics.isLowContent ? 30 : 40) ? 'great' : computed.acos <= (metrics.isLowContent ? 45 : 65) ? 'warning' : 'critical',
          target: metrics.isLowContent ? '< 30%' : '< 40%',
          hint: 'Ad Spend ÷ Ad Sales'
        },
        {
          label: metrics.isLowContent ? 'Max Breakeven CPC' : 'Total ACoS (TACoS)',
          value: metrics.isLowContent ? `$${computed.maxBreakevenCpc || 0.25}` : `${computed.tacos}%`,
          status: metrics.isLowContent 
            ? (computed.cpc <= (computed.maxBreakevenCpc || 0.25) ? 'great' : 'critical')
            : (computed.tacos <= 25 ? 'great' : computed.tacos <= 45 ? 'warning' : 'critical'),
          target: metrics.isLowContent ? `Afford ≤ $${computed.maxBreakevenCpc || 0.25}/click` : '< 25%',
          hint: metrics.isLowContent ? 'Conversion Rate × Net Royalty' : 'Spend ÷ Total Royalties'
        },
        {
    ...
    metrics.isLowContent
      ? {
          id: 'engagement',
          title: '4. Interior Layout & A+ Content',
          score: kenpScore,
          status: getScoreStatus(kenpScore),
          headline: metrics.hasAplusContent
            ? 'A+ Content Active (Interior Visuals Published)'
            : 'Missing A+ Content (Amazon restricts "Look Inside" for low-content!)',
          benchmark: 'Target: A+ Content with 3+ interior mockups & paper specs',
          valueDisplay: metrics.hasAplusContent ? 'A+ Content Active' : 'No A+ Content (Critical Leak)',
          whatYouAreMissing: metrics.hasAplusContent
            ? 'Your listing demonstrates interior paper layout, prompts, and line grid spacing. Keep an eye on customer reviews for feedback on paper weight or bleed.'
            : 'Because Amazon restricts the "Look Inside" feature on low-content books, shoppers cannot see your interior pages! Without A+ Content mockups, your conversion will stay severely depressed.',
          prescription: metrics.hasAplusContent
            ? 'Test seasonal A+ banners (e.g., Q4 gift guide, New Year resolution theme) to maximize Q4 low-content surges.'
            : 'Publish an Amazon A+ Content standard comparison grid showcasing 3 sample interior pages, prompt layouts, and paper specs immediately.',
          subMetrics: [
            {
              label: 'A+ Content Status',
              value: metrics.hasAplusContent ? 'Published' : 'Missing',
              status: metrics.hasAplusContent ? 'great' : 'critical',
              target: 'Published',
              hint: 'Interior mockups on Amazon page'
            },
            {
              label: 'KDP Print Cost',
              value: `$${computed.printCost || 2.80}`,
              status: (computed.printCost || 2.80) <= 2.50 ? 'great' : (computed.printCost || 2.80) <= 3.20 ? 'warning' : 'critical',
              target: '< $2.50',
              hint: 'Base print cost per copy'
            },
            {
              label: 'Net Royalty / Copy',
              value: `$${computed.royaltyPerUnit || 2.00}`,
              status: (computed.royaltyPerUnit || 2.00) >= 2.50 ? 'great' : (computed.royaltyPerUnit || 2.00) >= 1.75 ? 'warning' : 'critical',
              target: '≥ $2.00 / copy',
              hint: '(Price * 60%) - Print Cost'
            }
          ]
        }
      : {
